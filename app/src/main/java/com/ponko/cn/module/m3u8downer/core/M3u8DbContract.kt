...
class M3u8DbContract {
    ...
    class Dao(private var db: SQLiteDatabase?) {
        ...
        fun delete(m3u8: String) {
            if (exists(m3u8)) {
                ...
                BKLog.e(TAG, "插入${m3u8}对应数据失败，数据库不存在该条记录")
            }
        }
    ...
    class DaoBean {
        ...
        override fun toString(): String {
            return "DaoBean(m3u8='$m3u8', need_download_ts='$need_download_ts', not_download_ts='$not_download_ts', progress=$progress, total=$total, complete=$complete)"
        }
    }
}